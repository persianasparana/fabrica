...
// Reordena um corte (a ordem aqui = ordem das colunas na Ordem de Corte)
function epMoveCorte(i, dir) {
  const j = i + dir;
  if (!epCortes[i] || j < 0 || j >= epCortes.length) return;
  [epCortes[i], epCortes[j]] = [epCortes[j], epCortes[i]];
  renderEpCortes();
}

function renderEpCortes() {
  const el = document.getElementById('ep-cortes-lista'); if (!el) return;
  const GRID = '34px 1.3fr 1fr 38px 50px 56px 1fr 22px';
  const cab = epCortes.length ? `<div style="display:grid;grid-template-columns:${GRID};gap:4px;font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:.03em;margin-bottom:3px">
      <span title="posição — define a ordem das colunas na Ordem de Corte">Pos.</span><span>Parte</span><span>Fórmula</span><span>Dim</span><span>Qtd</span><span title="metragem da barra em metros — preencha só para materiais cortados de barra (tubo, perfil). Habilita o plano de corte.">Barra (m)</span><span>Setor</span><span></span>
    </div>` : '';
  el.innerHTML = cab + (epCortes.length ? epCortes.map((c,i)=>`
    <div style="display:grid;grid-template-columns:${GRID};gap:4px;align-items:center;margin-bottom:5px">
      <span style="display:flex;gap:1px">
        <button class="btn btn-outline" style="padding:1px 4px;font-size:10px;line-height:1.4${i===0?';opacity:.3;cursor:default':''}" onclick="epMoveCorte(${i},-1)" title="mover para cima (coluna mais à esquerda na ficha)" ${i===0?'disabled':''}>↑</button>
        <button class="btn btn-outline" style="padding:1px 4px;font-size:10px;line-height:1.4${i===epCortes.length-1?';opacity:.3;cursor:default':''}" onclick="epMoveCorte(${i},1)" title="mover para baixo (coluna mais à direita na ficha)" ${i===epCortes.length-1?'disabled':''}>↓</button>
      </span>
