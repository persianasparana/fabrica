<?php
/**
 * Bootstrap de container (Docker) — Backend do PCP.
 *
 ...
 *   PCP_ADMIN_USER, PCP_ADMIN_PASSWORD, PCP_ADMIN_NAME  (admin inicial)
...
$user = getenv('PCP_ADMIN_USER');
$pass = getenv('PCP_ADMIN_PASSWORD');
if ($user && $pass) {
    $count = (int) $db->fetchOne('SELECT COUNT(*) AS c FROM users')['c'];
    if ($count === 0) {
        $auth->createUser($user, $pass, getenv('PCP_ADMIN_NAME') ?: $user, 'admin');
