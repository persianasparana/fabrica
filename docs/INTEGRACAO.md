...
0. **API de integração por API key** (implementada — usada pela Logística).
   Rotas **somente leitura** sob `/api/integracao/*`, autenticadas pelo header
   `X-API-Key` (sem sessão/CSRF). A chave vive em `INTEGRACAO_API_KEY` no `.env`
   (gerar com `openssl rand -hex 32`); sem a variável, as rotas respondem **503**.
